...
def evaluate_model_batchwise(model_view1, model_view2, dataloader, batch_size=512):
    """
    Efficient batch-wise evaluation for Recall@1 and MRR.
    """
    model_view1.eval()
    model_view2.eval()
    ...
    for i in tqdm(range(0, num_samples, batch_size), desc="Computing metrics"):
        ...
        scores = torch.mm(batch_emb, all_emb_view2.t())
...
def train_and_evaluate(train_csv, val_csv, epochs=3, start_epoch=1, batch_size_train=64, batch_size_val=128, lr=2e-5,
                       temperature=0.07, max_length=128, num_workers=8):
    # Dataset and Loader
    ...
    if resume_checkpoint:
        model_view1.load_state_dict(torch.load(f"best_model_view1_1.pt", map_location=device))
        model_view2.load_state_dict(torch.load(f"best_model_view2_1.pt", map_location=device))
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for batch in tqdm(train_loader, desc=f"Training Epoch {epoch}/{epochs}"):
            view1_ids = batch["view1_input_ids"].to(device, non_blocking=True)
            view1_mask = batch["view1_attention_mask"].to(device, non_blocking=True)
            view2_ids = batch["view2_input_ids"].to(device, non_blocking=True)
            view2_mask = batch["view2_attention_mask"].to(device, non_blocking=True)
        ...
        # ---- Save checkpoint ----
        torch.save(model_view1.state_dict(), f"best_model_view1_{epoch}.pt")
        torch.save(model_view2.state_dict(), f"best_model_view2_{epoch}.pt")
        ...
        print(f"Epoch {epoch} - Validation Recall@1: {recall_at_1:.4f}, MRR: {mrr:.4f}")
        ...
        if recall_at_1 > best_recall:
            ...
            best_epoch = epoch
            torch.save(model_view1.state_dict(),
                       "../outputs/sbert_mps/run_20250402_224539/checkpoints/best_model_view1.pt")
            torch.save(model_view2.state_dict(),
                       "../outputs/sbert_mps/run_20250402_224539/checkpoints/best_model_view2.pt")
...
def test_evaluate(test_csv, max_length=128, batch_size=128, num_workers=8):
    ...
    model_view1 = DualEncoderSBERT().to(device)
    model_view2 = DualEncoderSBERT().to(device)
    model_view1.load_state_dict(torch.load("../outputs/sbert_mps/run_20250402_224539/checkpoints/best_model_view1.pt", map_location=device))
    model_view2.load_state_dict(torch.load("../outputs/sbert_mps/run_20250402_224539/checkpoints/best_model_view2.pt", map_location=device))
...
if __name__ == '__main__':
    ...
    train_csv = "data/filtered_splits/electronics_train.csv"
    val_csv = "data/filtered_splits/electronics_val.csv"
    test_csv = "data/filtered_splits/electronics_test.csv"

    train_and_evaluate(train_csv, val_csv, epochs=3, start_epoch=2, batch_size_train=64, batch_size_val=128, lr=2e-5,
                       temperature=0.07, max_length=128, num_workers=8)

    test_evaluate(test_csv, max_length=128, batch_size=128, num_workers=8)
